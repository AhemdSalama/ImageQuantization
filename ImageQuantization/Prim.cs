        ...
        public double MstPrim(List<RgbPixel> distinctColors)    // Θ(V^2)
        {
            ...
            for (var i = 0; i < _VertixSize - 1; i++) // Θ(v * body)
            {
                ...
                for (var j = 0; j < _VertixSize; j++)   // Θ(v * body)
                {
                    ...
                    {
                        var cost = RgbPixel.EuclideanDistanceSquared(distinctColors[startNode], distinctColors[j]);    // Θ(1)
                        if (cost < _key[j]) // Θ(1)
                        {
                ...
                mstCost += Math.Sqrt(_key[startNode]); // Θ(1)
                edges[idx] = (new Edge(startNode, _parent[startNode], _key[startNode])); // Θ(1)
                idx++;
            }
